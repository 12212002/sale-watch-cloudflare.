...
export async function retrieve(url){
 const u=new URL(url);if(u.protocol!=='https:'||u.hostname!=='www.lisagozlan.com'||u.username||u.password||u.port)throw new Error('Unsafe retailer URL.');
...
export function robotsAllow(body,path){
 const groups=[];let agents=[],rules=[],haveRules=false;
 for(const raw of body.split(/\r?\n/)){const line=raw.split('#')[0].trim();const m=line.match(/^([^:]+):\s*(.*)$/);if(!m)continue;const key=m[1].toLowerCase(),value=m[2].trim();if(key==='user-agent'){if(haveRules){groups.push({agents,rules});agents=[];rules=[];haveRules=false;}agents.push(value.toLowerCase());}else if(['allow','disallow'].includes(key)){haveRules=true;if(value)rules.push({allow:key==='allow',value});}}
 groups.push({agents,rules});let matching=groups.filter(g=>g.agents.some(a=>a!=='*'&&UA.toLowerCase().startsWith(a)));if(!matching.length)matching=groups.filter(g=>g.agents.includes('*'));
 const matched=matching.flatMap(g=>g.rules).filter(r=>{const exp=r.value.split('*').map(v=>v.replace(/[.+?^{}()|[\]\\]/g,'\\$&')).join('.*');return new RegExp('^'+exp).test(path);}).sort((a,b)=>b.value.length-a.value.length||Number(b.allow)-Number(a.allow));return matched[0]?.allow??true;
}
export function parseLisa(html,json,variantId,handle,now=new Date()){
 ...
 const products=nodes.filter(n=>n['@type']==='Product'&&new URL(n.url,'https://www.lisagozlan.com').pathname.replace(/\/$/,'')==='/products/'+handle);
 ...
 const matches=offers.filter(o=>{try{const u=new URL(o.url);return u.hostname==='www.lisagozlan.com'&&u.pathname==='/products/'+handle&&u.searchParams.get('variant')===String(v.id)&&o.sku===v.sku;}catch{return false;}});
 ...
 return {...regularPrice(v.compare_at_price,v.price,{productId,variantId:vid},'shopify-exact-variant-compare-at'),...controls,state:'VERIFIED',retailer:'lisa-gozlan',productId,variantId:vid,seller:'Lisa Gozlan',currency:'CAD',cents,available:true,title:String(json.title),variant:String(v.title),imageUrl:productImage(v.featured_image?.src||json.featured_image),method:'Variant-specific JSON-LD + public Shopify product JSON + Canadian market context',signals:['json-ld-offer','shopify-product-json'].map(source=>({source,kind:'selling-price',productId,variantId:vid,currency:'CAD',cents}))};
}
let policy=null;
export async function inspectLisa(url){
 try{
 ...
 if(!policy||Date.now()-policy.at>86400000)policy={body:await retrieve('https://www.lisagozlan.com/robots.txt'),at:Date.now()};
 ...
 const html=await retrieve('https://www.lisagozlan.com'+page+(variant?'?variant='+encodeURIComponent(variant):''));
 if(isChallenge(html))return {state:'BLOCKED',reason:'Retailer check blocked.'};
 const json=JSON.parse(await retrieve('https://www.lisagozlan.com'+page+'.js'));
 return parseLisa(html,json,variant,handle);
